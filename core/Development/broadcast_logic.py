...
class CustomTensor:
    ...
    # def _reduce_grad_for_broadcast(self, grad, target_shape):
    #     """
    #     Reduces the gradient tensor 'grad' to 'target_shape' by summing
    #     along broadcasted dimensions.
    #     """
    #     if grad.shape == target_shape:
    #         return grad

    #     # Determine dimensions to sum over
    #     # PyTorch broadcasting rule: compare shapes from right to left.
    #     # If dimensions mismatch or one is 1, and the other is >1,
    #     # the dimension of size 1 is broadcasted.
    #     # Gradients for broadcasted dimensions must be summed.

    #     # Pad target_shape with 1s on the left to match grad's ndim
    #     padded_target_shape = (1,) * (grad.ndim - len(target_shape)) + target_shape

    #     # Dimensions to sum over are those where grad.shape[i] != padded_target_shape[i]
    #     # AND padded_target_shape[i] == 1
    #     sum_dims = []
    #     for i in range(grad.ndim):
    #         if grad.shape[i] != padded_target_shape[i] and padded_target_shape[i] == 1:
    #             sum_dims.append(i)
        
    #     if sum_dims:
    #         grad = grad.sum(dim=sum_dims, keepdim=True)
        
    #     # Squeeze dimensions that were originally 1 in target_shape
    #     # but became >1 in grad.
    #     # Example: target (2,), grad (3,2). Summed to (1,2) -> squeeze to (2,)
    #     for i in range(len(padded_target_shape)):
    #         if padded_target_shape[i] == 1 and grad.shape[i] > 1: # if we summed over it, it was keepdim=True
    #              grad = grad.squeeze(i) # Squeeze removes the 1-sized dimension

    #     # Handle cases where target_shape has fewer dimensions (e.g., matrix + vector)
    #     while grad.ndim > len(target_shape):
    #         grad = grad.sum(dim=0) # Sum the leading dimensions
