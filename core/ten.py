...
class CustomTensor(torch.Tensor):
    def __new__(cls,graph=None,save_strong_reference=None,data=None,requires_grad=False,_tensor=None,_is_leaf=False,dtype=None,device=device):
        if data and _tensor:
            raise ValueError("Only one of 'data' or '_tensor' can be provided.")
        if requires_grad and not graph:
            raise ValueError("Graph instance must be provided for tensors which require gradients")
        if requires_grad and not save_strong_reference:
            raise ValueError("save_strong_reference must be provided for tensors which require gradients")
        if isinstance(data,CustomTensor):
            warnings.warn(f"{data} is already a Custom Tensor Object")
            return data
        if isinstance(_tensor,CustomTensor):
            warnings.warn(f"{_tensor} is already a Custom Tensor Object")
            return _tensor  
        ...
        instance.node_id = graph.add_tensor(weakref.ref(instance)) if requires_grad else None
